use crate::models::{CreateToolRequest, InvokeToolRequest, InvokeToolResponse, Tool, ToolMetadata, AuthorizeRequest, SetEnvRequest, CallerIdentity};
...
pub async fn invoke_tool(
    ...
    match caller {
        ...
        CallerIdentity::Agent(ref name) => {
            if !tool.allowed_agents.contains(name) && tool.author_agent != *name {
                return Json(InvokeToolResponse {
                    success: false,
                    output: None,
                    error: Some(format!("Agent '{}' is not authorized to invoke this tool", name)),
                });
            }
        }
    }

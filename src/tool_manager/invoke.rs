...
fn build_bwrap_command(
    ...
    cmd.args([
        ...
        "--dir", "/etc/ssl/certs",
        "--ro-bind", "/etc/static/ssl/certs/ca-bundle.crt", "/etc/ssl/certs/ca-bundle.crt",
        "--ro-bind", "/etc/static/ssl/certs/ca-certificates.crt", "/etc/ssl/certs/ca-certificates.crt",
        "--dev", "/dev",
        "--proc", "/proc",
        "--tmpfs", "/tmp",
        "--die-with-parent",
...
mod tests {
    ...
    #[test]
    fn test_find_bwrap_returns_pathbuf() {
        let result = find_bwrap();
    ...
    fn test_build_bwrap_command_basic() {
        ...
        let tool_dir = dir.path().to_path_buf();
        let bwrap = PathBuf::from("bwrap");
        ...
        let debug_format = format!("{:?}", cmd);
        assert!(debug_format.contains("bwrap"));
    }

    #[test]
    fn test_build_bwrap_command_with_closure() {
        let dir = tempdir().unwrap();
        let tool_dir = dir.path().to_path_buf();
        let bwrap = PathBuf::from("bwrap");
        let env_vars = std::collections::HashMap::new();
        let closure = vec![
            PathBuf::from("/nix/store/abc123-pkg"),
            PathBuf::from("/nix/store/def456-other"),
        ];
        ...
        let debug_format = format!("{:?}", cmd);
        assert!(debug_format.contains("abc123-pkg"));
        assert!(debug_format.contains("def456-other"));
    }

    #[test]
    fn test_build_bwrap_command_with_env_vars() {
        let dir = tempdir().unwrap();
        let tool_dir = dir.path().to_path_buf();
        let bwrap = PathBuf::from("bwrap");
        ...
        let debug_format = format!("{:?}", cmd);
        assert!(debug_format.contains("API_KEY"));
        assert!(debug_format.contains("secret"));
    }

    #[test]
    fn test_build_bwrap_command_with_shell_env() {
        let dir = tempdir().unwrap();
        let tool_dir = dir.path().to_path_buf();
        let bwrap = PathBuf::from("bwrap");
        let env_vars = std::collections::HashMap::new();
        let shell_env = ShellEnv {
            path: "/nix/store/some-path/bin".to_string(),
        };
        ...
        let debug_format = format!("{:?}", cmd);
        assert!(debug_format.contains("PATH"));
        assert!(debug_format.contains("/nix/store/some-path/bin"));
    }

    #[test]
    fn test_build_bwrap_command_entrypoint_with_args() {
        let dir = tempdir().unwrap();
        let tool_dir = dir.path().to_path_buf();
        let bwrap = PathBuf::from("bwrap");
        let env_vars = std::collections::HashMap::new();
        ...
        let debug_format = format!("{:?}", cmd);
        assert!(debug_format.contains("python"));
        assert!(debug_format.contains("script.py"));
    }

    #[test]
    fn test_build_bwrap_command_input_json_passed() {
        let dir = tempdir().unwrap();
        let tool_dir = dir.path().to_path_buf();
        let bwrap = PathBuf::from("bwrap");
        let env_vars = std::collections::HashMap::new();
        let input = r#"{"key":"value","nested":{"item":1}}"#;
    ...
    #[test]
    fn test_build_bwrap_command_binds_tool_dir() {
        let dir = tempdir().unwrap();
        let tool_dir = dir.path().to_path_buf();
        let bwrap = PathBuf::from("bwrap");
        let env_vars = std::collections::HashMap::new();
        ...
        let debug_format = format!("{:?}", cmd);
        assert!(debug_format.contains("/tool"));
    }

    #[test]
    fn test_shell_env_debug() {
        let env = ShellEnv {
            path: "/usr/bin:/bin".to_string(),
        };
        let debug = format!("{:?}", env);
        assert!(debug.contains("ShellEnv"));
    }

    #[test]
    fn test_build_bwrap_command_empty_closure() {
        let dir = tempdir().unwrap();
        let tool_dir = dir.path().to_path_buf();
        let bwrap = PathBuf::from("bwrap");
        let env_vars = std::collections::HashMap::new();
        ...
        let debug_format = format!("{:?}", cmd);
        assert!(debug_format.contains("/nix/store"));
    }

    #[test]
    fn test_build_bwrap_command_multiple_env_vars() {
        let dir = tempdir().unwrap();
        let tool_dir = dir.path().to_path_buf();
        let bwrap = PathBuf::from("bwrap");
        let mut env_vars = std::collections::HashMap::new();
        ...
        let debug_format = format!("{:?}", cmd);
        assert!(debug_format.contains("VAR1"));
        assert!(debug_format.contains("VAR2"));
        assert!(debug_format.contains("VAR3"));
    }

    #[test]
    fn test_build_bwrap_command_python_entrypoint() {
        let dir = tempdir().unwrap();
        let tool_dir = dir.path().to_path_buf();
        let bwrap = PathBuf::from("bwrap");
        let env_vars = std::collections::HashMap::new();
        ...
        let debug_format = format!("{:?}", cmd);
        assert!(debug_format.contains("python"));
        assert!(debug_format.contains("main.py"));
    }

    #[test]
    fn test_build_bwrap_command_complex_closure() {
        let dir = tempdir().unwrap();
        let tool_dir = dir.path().to_path_buf();
        let bwrap = PathBuf::from("bwrap");
        let env_vars = std::collections::HashMap::new();
        let closure = vec![
            PathBuf::from("/nix/store/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa-python-3.11"),
            PathBuf::from("/nix/store/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb-requests-2.28"),
            PathBuf::from("/nix/store/cccccccccccccccccccccccccccccccc-openssl-3.0"),
        ];
        ...
        let debug_format = format!("{:?}", cmd);
        assert!(debug_format.contains("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"));
        assert!(debug_format.contains("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"));
        assert!(debug_format.contains("cccccccccccccccccccccccccccccccc"));
    }
}